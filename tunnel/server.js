...
const { tryParseJSON, log, types, verifyDataJson } = require("./utils");
const { CLIENT, AGENT } = types;

const portsFrom = parseInt(process.env.N_T_SERVER_PORTS_FROM) || 3005;
const portsTo = parseInt(process.env.N_T_SERVER_PORTS_TO) || 3009;
const ports = Array(1 + portsTo - portsFrom)
  .fill()
  .map((e, i) => i + portsFrom);
...
const serviceServer = net.createServer((serviceSocket) => {
  const onData = (dataEnc) => {
    // known agent or client, sending pong
    if (serviceSocket.cProps && serviceSocket.cProps.uuid) {
      return serviceSocket.write(`{"pong":${Math.random()}}`);
    }

    // try decrypt otherwise - kill
    const data = dataEnc.toString("utf8");
    ...
    if (connections[dataJson.name][dataJson.type][dataJson.uuid]) {
      ...
      const deadSocket = connections[dataJson.name][dataJson.type][dataJson.uuid].socket;
    ...
    if (dataJson.type === CLIENT) {
      ...
      const agent = connections[dataJson.name][AGENT];
      ...
      if (!agentObj.port) {
        ...
        if (!agentObj.port) {
          return serviceSocket.destroy();
        } // todo notify agent that there are no free ports
...
serviceServer.on("listening", (listener) =>
  log.info("Server listening on port", serviceServerPort)
);
serviceServer.on("error", (err) => {
  log.info(
    "Something went wrong with service server. Stopping...\n",
    err.name || err.code,
    err.message
  );
  serviceServer.close();
...
const createServer = (connectionName, serviceAgentUuid) => {
  let agentSockets = [];
  let clientSockets = [];
  ...
  pipes[connectionName].server = net.createServer((socket) => {
    socket.once("data", (dataEnc) => {
      // try decrypt otherwise - kill
      let data = dataEnc.toString("utf8");
      ...
      if (dataJson.type === AGENT) {
        ...
        if (clientSockets.length > 0) {
          ...
          // clientSocket.write("" + Math.random()); // just something, it doesn't matter for now
        } else agentSockets.push(socket);
      }
      // client
      else if (dataJson.type === CLIENT) {
    ...
    socket.on("close", (error) => {
      ...
      if (conPipes[socket.uuid].socket) {
        ...
        if (!conPipes[socket.uuid].socket.destroyed) {
          conPipes[socket.uuid].socket.destroy();
        }
      }
  ...
  pipes[connectionName].server.on("listening", (listener) =>
    log.info(`Agent "${connectionName}" connected, dedicated port ${dataServerPort}`)
  );
  pipes[connectionName].server.on("error", (err) => {
    log.info(
      "Something went wrong with agent server. Killing agent...\n",
      err.name || err.code,
      err.message
    );
...
const notify = (socket, port, uuid) => {
  return socket && !socket.destroyed && socket.write(`{"port":${port},"uuid":"${uuid}"}`);
};
