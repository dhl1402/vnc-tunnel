...
const agentName = process.env.N_T_AGENT_NAME || 'test-vnc'
...
const pipePort = parseInt(process.env.N_T_AGENT_DATA_PORT) || 5900
  ...
  dataArr.forEach(value => {
    ...
    dataAgent.on('connect', () => {
      ...
      localSocket.on('close', () => {
        ...
        if (isPiped) {
          dataAgent.unpipe(localSocket).unpipe(dataAgent)
          isPiped = false
        }
...
serviceAgent.on('connect', () => {
  ...
  pinger = setInterval(() => {
    serviceAgent.write('' + Math.random())
  }, 15000)
})
